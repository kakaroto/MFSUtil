...
class CFG(object):
  ...
  def __init__(self, data):
    ...
    for i in xrange(self.num_records):
        ...
        if record.name == "..":
          assert len(path) > 1
          file = self.getFile(os.path.join(*path))
          assert file and file.isDirectory()
          assert file.record.mode == record.mode
          path.pop()
        else:
          file = CFGFile(os.path.join(*path + [record.name]), record, self.data, parent)
          self.files.append(file)
          if record.isDirectory():
            path.append(record.name)
        parent = self.getFile(os.path.join(*path))
  ...
  def __str__(self):
    ...
    path = ["/"]
    for record in self.records:
      ret += "%s %s\n" % (str(record), os.path.join(*path + ([] if record.name == ".." else [record.name])))
      if record.name == "..":
        path.pop()
